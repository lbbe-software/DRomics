test_that("bmdcalc works as expected on the BMD results",
  {
    ...
    expect_equal(sum(is.nan(BMD.xfold.2)), 12)
    expect_equal(sum(is.na(BMD.zSD.1)), 0)
    expect_equal(sum(is.na(BMD.zSD.2)), 5)
    expect_equal(sum(is.na(BMD.xfold.1)), 31)
    expect_equal(sum(is.na(BMD.xfold.2)), 66)
    expect_equal(round(mean(BMD.zSD.1, na.rm = TRUE), 4), 2.1107)
    expect_equal(round(mean(BMD.zSD.2, na.rm = TRUE), 4), 3.9974)
    expect_equal(round(mean(BMD.xfold.1, na.rm = TRUE), 4), 3.2778)
    expect_equal(round(mean(BMD.xfold.2, na.rm = TRUE), 4), 5.1575)
