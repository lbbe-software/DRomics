...
that is provided on the last page of the Shiny application. 
...
(for example, if there are triplicates for each treatment, the first line could be "item", 0, 0, 0, 0.1, 0.1, 0.1, etc.). This file is imported within DRomics using the function read.table() with its default field separator (sep argument). 
...
# Here the argument backgrounddose is used to specify that doses below 0.1
# are considered can be fixed at 0 in the DRomics workflow.
