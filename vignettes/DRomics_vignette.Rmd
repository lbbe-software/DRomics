...
other **continuous omics data** such as metabolomics or proteomics (in log scale). In order to link responses across biological levels based on a common method, DRomics also handles **apical data** as long as they are **continuous and follow a Gaussian (normal) distribution** for each dose or concentration, with a **constant standard error**.

DRomics does not provide an annotation pipeline, but provides **functions to help the interpretation** of the workflow results in view of a **common biological annotation** (obtained outside of DRomics using databases such as KEGG or Gene Ontology). It enables a **multi-omics approach**, with the comparison of
the response at the different levels of organization. It can also be used to **compare the response** at one organization level, but measured under **different experimental conditions** (e.g. different time points). 
...
compatible with a Gaussian (normal) error distribution (e.g. after logarithmic transformation) for the 
selection and modeling steps.
...
Gaussian error model**. This strong hypothesis is required both for selection of items and for dose-response modeling.
...
It is also possible to handle **experimental data collected using a design with a batch effect** using
...
to correct for this batch effect before selection and modeling steps.  We also developed the PCAplot()
function to help visualizing this batch effect and the impact of the **batch effect correction (BEC)** on data
...
and **functions from specific packages** can be used to **correct for the batch effect**.
...
(or other continuous omic data) and **RNAseq data** (as **sva** is a Bioconductor package 
it must be installed in the same as **DESeq2** and **limma** - if needed look at the DRomics
web page to get the good instruction: 
...
Bayesian approach** using the respective packages **DESeq2** and **limma** for **RNAseq data** and 
...
For Step 3 the function drcfit() must be simply used with the output of itemselect() as first argument.
For each item selected in Step 2, the model that best fits the dose-response data is chosen among 
a **family of five simple models built to describe a wide variety of monotonic and biphasic dose-response curves (DRC)** (and exclusively monotonic and biphasic curves : it why 
...
As the call to this the drcfit() function may take time, by default a progressbar is provided.
...
* the **linear model**: $$y = d + bx$$ with **2 parameters**,  $b$ the slope and $d$ the mean signal at control. 
* the **exponential model**: $$y = d + b(exp(\frac{x}{e})-1)$$ with **3 parameters**,
...
$c$ the asymtotic signal for high doses, $d$ the mean signal at control, and $e$ the dose at the inflection point ($>0$).
* the **Gauss-probit model**: $$y = fexp(-0.5(\frac{x-e}{b})^2)+d+(c-d)\Phi(\frac{x-e}{b})$$ 
with **5 parameters**,
...
$e$ ($>0$) a shape parameter corresponding to the mean of the Gaussian part, and 
$f$ the amplitude of the Gaussian part, which is U-shape if $f<0$ and bell-shape if $f>0$.
$\Phi$ represents the cumulative distribution function (CDF) of the standard normal distribution.
This model encompasses **two simplifed versions with 4 parameters**, one **monotonic** when $f=0$ and one with **symmetrical asymptotes** (when $c=d$).
* the **log-Gauss-probit model**: $$y = fexp(-0.5(\frac{ln(x)-ln(e)}{b})^2)+d+(c-d)\Phi(\frac{ln(x)-ln(e)}{b})$$ 
$\Phi$ representing the cumulative distribution function (CDF) of the standard normal distribution,
with **4 or 5 parameters** among
$b$ ($>0$) a shape parameter corresponding to the standard deviation
of the Gaussian part, 
$c$ the asymtotic signal for high dose,
$d$ the asymptotic signal on the left of the DRC, reached at the control (for $ln(x) = ln(0) = -\infty$),
$ln(e)$ ($>0$) a shape parameter corresponding to the mean of the Gaussian part, and 
$f$ the amplitude of the Gaussian part, which is U-shape if $f<0$ and bell-shape if $f>0$.
As the previous one, this model encompasses **two simplifed versions with 4 parameters**, one **monotonic** when $f=0$ and one with **symmetrical asymptotes** (when $c=d$).
...
It is why the scale under which you should import your data is very important, as a log (or pseudo-log) transformation
is necessary to meet the use conditions of the model for some types of data.
...
$N(0, \sigma)$ representing the Gaussian (normal) distribution of standard deviation (SD) $\sigma$.
...
that the error model is not strongly violated.
...
tracenormale <- function(indice)
{
  ...
  lines(coefsurx*xplot+x, yplot, col = "red", lwd = 2)
...
The **two types of benchmark doses (BMD-zSD and BMD-xfold) proposed by the EFSA (2017) are systematically calculated**
...
(or see ?bmdcalc for details on this function).
...
by default at 10 (for 10%), is used to define the BMD-xfold as the dose at which the response is reaching the BMR defined as $BMR = y_0 \pm \frac{x}{100} \times y_0$.
...
on our example. An extensive description of the outputs of the complete DRomics workflow is provided in the 
...
plot by "trend", "model" or "typology". 
...
and it is note the default option) to scale the signal
...
This section illustrates functions of DRomics that are meant to help the biological interpretation of outputs. The idea is to **augment the output data frame with a new column bringing biological information** such as provided by **biological annotation** of the items (e.g. kegg pathway classes or GO terms) then to **use this information to organize the visualisation** of the DRomics output.
...
Nevertheless it is possible to perform it without R coding within the **Shiny application DRomicsInterpreter-shiny**. 
Report to the [introduction section](#introduction) to see how to launch the Shiny application.

An example of how to proceed:

1. **Import the data frame with DRomics results to be used: the output `$res` of bmdcalc() or bmdboot() functions from Step 4 of the main DRomics workflow.**

(This step will not be necessary if previous steps are done directly in R using the DRomics package as described previously in this vignette
(see the [section describing this output data frame](#outputs)). We did it to take a real example that took a long time to run but from which results are stored in the package.)
...
# to see the first lines of this file
head(res)
```
...
Examples are embedded in the DRomics package, but be cautious, generally this file must be produced by the user. Each item may have more than one annotation (*i.e.* more than one line). If you are in a case where the biological annotation was done before the dose-response modeling, 
so **if items were annotated whatever they were selected by the DRomics workflow or not**, 
you might **previously reduce the dimension of your annotation file by selecting only the items selected by the DRomics workflow**. 
...
It is also possible to show a **summary of BMD values in each pathway/category** as a **chosen summary** 
(argument BMDsummary which can be **"first.quartile", "median" or "median.and.IQR"** for
...
This section illustrates functions of DRomics that are meant to help the interpretation of outputs by taking into account **different experimental levels (different molecular levels, different time points, different experimental settings, ...)**. The idea is to perform
...
to bind the augmented data frames and add a column coding for the experimental level and to use this column
...
Following the same steps as described before [for one level](#augmentation), below is an example of R code to import the DRomics results for microarray data, and to merge them with information on biological annotation.
...
(argument nitems, to select the **most represented groups**) and/or **on the BMD summary value** 
(argument BMDmax, to select the **most sensitive groups**). 
