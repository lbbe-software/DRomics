...
("rlog" or "vst"). "rlog" is recommended and chosen by default but more computer intensive than "vst" (see ?RNAseqdata for details). Whatever the chosen method, data are automatically normalized with respect to library size and transformed in a log2 scale.


```{r}
(o.RNAseq <- RNAseqdata(RNAseqfilename, transfo.method = "rlog"))
...
normalization method ("cyclicloess", "quantile", "scale" or "none"). "cyclicloess" is recommended and chosen by default but more computer intensive than the others (see ?microarraydata for details).
...
```{r}
(o.microarray <- microarraydata(microarrayfilename, norm.method = "cyclicloess"))
...
The false discovery rate (FDR) must be chosen (we recommend a value between 0.001 and 0.1 depending of the initial number of items). 
...
For Step 3 the function drcfit() must be simply used with the output of itemselect() as first argument. Description of the fitted models and of the procedure to select the best fit are described in Larras et al. 2018 and in ?drcfit.
...
are represented by solid circles. All the fitted curves are
saved in a pdf file (see argument saveplot2pdf in ?drcfit).
...
be used ("dose_residuals" or "fitted_residuals")
...
Both types of benchmark doses (BMD-zSD and BMD-xfold) are systematically calculated
...
### Various plots of BMD distribution
...
In the example below a small number of iterations was used to make this vignette quick to compile, but the default value of
the argument niter (1000) should be considered as a minimal value to
obtain stable results.

### Bootstrap calculation
```{r}
(b <- bmdboot(r, niter = 100, progressbar = FALSE))
```
...
# 2. Import the dataframe with functional annotation (or any other descriptor/category you want to use, here KEGG pathway classes) 
