...
#### Importation of data in a unique text file {#textfile}
...
(see ?RNAseqdata and the [section dedicated to in situ data](#insitudata) for details on this point). 
...
In that example the argument backgrounddose is used to specify that doses below  or equal to 0.1
are considered as 0 in the DRomics workflow.
Specifying this argument is necessary when there are no doses at 0 in the data (see [section
...
for which there are **no replicates, as the dose (or concentration) is not controlled**
...
For the second step of the workflow, function itemselect() must be used with the output of the function used in [step 1](#step1) as first argument (output of RNAseqdata(), microarraydata(), continuousomicdata() or continuousanchoringdata()). Below is an example with microarray data.
...
DRomicsInterpreter-shiny can be used to implement all the steps described in this vignette without to code them in R. But in any case, the biological annotation of items selected in the first DRomics workflow must have been done using a database such as the Gene Ontology (GO) of the kegg databases.
...
One can focus on a group of interest,
as below for the group "Lipid metabolism", and add the labels of items
...
### Plot of dose-response curves for a specific biological group {#curvesplot}
...
### Comparison of results obtained at the different levels using basic R functions {#comparison}
...
### Selection on groups on which to focus using the selectgroups() function

When the number of biological groups obtained after annotation of items
selected in the deose-response modeling workflow is too high, it may be useful
to select groups on whch to focus, to enhance the visibility of plots.
...
use selection criteria on the number of items representing each biological
group (to select the most represented) and/or on the BMD summary value 
(to select the most sensitive groups). The selectgroups() function can be used for this purpose as in the example below.

!!!!!!!!!!!! revoir selectgroups pour que ça sélectionne tous les niveaux expérimentaux
!!!!!!!!!!!! si l'un respect les conditions 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!! J'en suis là !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
Especially as metabolic data and transcriptomic data were not imported
